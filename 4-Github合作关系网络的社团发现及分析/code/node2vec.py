...
class node2vec:
    ...
    def init_transition_prob(self):
        ...
        for node in g.nodes:
            ...
            probs = [int(g[node][n]['weight']) for n in nbs]
            # print('probs', probs)
            # Normalized
            norm = sum(probs)
    ...
    def get_alias_edge(self, t, v):
        ...
        for v_nbr in sorted(g.neighbors(v)):
            if v_nbr == t:
                unnormalized_probs.append(int(g[v][v_nbr]['weight']) / self.args['p'])
            elif g.has_edge(v_nbr, t):
                unnormalized_probs.append(int(g[v][v_nbr]['weight']))
            else:
                unnormalized_probs.append(int(g[v][v_nbr]['weight']) / self.args['q'])
